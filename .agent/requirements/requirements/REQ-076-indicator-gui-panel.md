...
### 指标选择器面板
- [ ] 新增"指标"面板，位于图表区域侧边或顶部
- [ ] 指标列表分类显示：趋势（SMA/EMA/MACD/ADX）、震荡（RSI/KDJ/CCI）、波动（ATR/布林带）、成交量（OBV/VWAP）
- [ ] 勾选指标即叠加到当前图表
- [ ] 支持同时显示多个指标
...
- [ ] RSI：周期 n（默认 14）
- [ ] 布林带：周期 n（20）、标准差 dev（2.0）
- [ ] ATR：周期 n（默认 14）
- [ ] 参数修改后实时重绘

### 图表叠加
- [ ] 趋势指标（SMA/EMA/布林带）叠加到主图 K 线
- [ ] 震荡指标（RSI/MACD/KDJ/CCI）显示在副图
- [ ] 指标线颜色可配置
- [ ] 指标数值实时显示在图例区域
- [ ] 右键指标可移除

### Python 联动
- [ ] REQ-069 的 PyArrayManager 指标计算与 GUI 指标面板共享数据
- [ ] Python 策略注册的指标自动出现在 GUI 指标面板
