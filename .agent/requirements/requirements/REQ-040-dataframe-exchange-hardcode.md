...
## 验收标准
- [ ] `convert_polars_to_bars()` 接受 exchange 参数（或从 DataFrame 列自动推断）
- [ ] `convert_polars_to_bars()` 接受 interval 参数（或从 DataFrame 列自动推断）
- [ ] 转换后的 BarData 包含正确的 exchange 和 interval 值
