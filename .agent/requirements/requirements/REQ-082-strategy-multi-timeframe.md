...
## 验收标准
- [x] 策略 `on_init` 中可调用 `context.subscribe_bars(vt_symbol, intervals)` 订阅多周期
- [x] 每个周期K线完成时触发 `on_bar`，`BarData` 包含 `interval` 字段标识周期
- [x] 支持从1分钟K线合成5分钟/15分钟/1小时等标准周期
- [x] `on_bars` 回调在所有订阅周期都更新时触发，传入 `HashMap<String, BarData>`
- [x] Binance `@kline_1m` WebSocket流优先使用，fallback到 `BarGenerator` tick聚合
- [x] 每个周期的 `ArrayManager` 独立维护，互不干扰
