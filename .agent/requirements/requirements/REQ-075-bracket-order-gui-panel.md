...
### 组合单列表面板
- [x] 新增"组合单"标签页，展示所有活跃组合单组
- [x] 列表显示：组ID、类型（Bracket/OCO/OTO）、合约、状态、入场价、止盈价、止损价
...
### Python API
- [x] `PyBracketOrderEngine` 类暴露 `add_bracket_order()` 方法
- [x] `PyBracketOrderEngine` 类暴露 `add_oco_order()` 方法
- [x] `PyBracketOrderEngine` 类暴露 `add_oto_order()` 方法
- [x] `PyBracketOrderEngine` 类暴露 `cancel_group()` 方法
- [x] `PyBracketOrderEngine` 类暴露 `get_active_groups()` 查询方法
- [x] Python 通过 `engine.bracket_order_engine` 访问
