...
versions:
  - version: 1
    ...
    author: ai
    context: "MCP 分析后发现当前仅 13 个工具。参考 OKX Agent Trade Kit (140工具)、MetaTrader MCP Server (32工具)，建议扩展到 40+ 个工具。用户确认需要参考项目工具集增强。"
    reason: "扩展 MCP 工具集，覆盖更多交易场景"
    snapshot: "将 MCP 工具从 13 个扩展到 40+ 个，参考 OKX/MetaTrader 分类"
---
...
当前 MCP Server 仅提供 13 个工具（6 个 Trading + 7 个 UI）。参考成熟的交易 MCP 项目，扩展到 40+ 个工具，覆盖更完整的交易场景。
...
| 模块 | 工具数 | 功能 |
|------|--------|------|
| market | 19 | Ticker, K线, 技术指标(70+), 市场筛选 |
| spot | 13 | 现货下单/撤单/批量 |
| swap | 17 | 永续合约交易/杠杆/追踪止损 |
| futures | 18 | 交割合约/条件单/OCO |
...
### News/Sentiment 分析（4个）

| 工具 | 描述 |
|------|------|
| `get_news` | 获取市场新闻 |
| `analyze_sentiment` | 分析情绪（使用 Sampling） |
...
- [ ] Market 工具：10个全部实现
- [ ] Trading 工具：从 4 个扩展到 12 个
- [ ] Account 工具：6个全部实现
- [ ] Strategy 工具：8个全部实现
- [ ] Risk 工具：5个全部实现
- [ ] Backtest 工具：4个全部实现
- [ ] News/Sentiment 工具：4个全部实现
- [ ] 工具文档：每个工具有清晰的描述和参数说明
- [ ] 错误处理：所有工具有统一的错误响应格式
- [ ] 权限控制：敏感操作需要确认

## 安全设计

参考 OKX Agent Trade Kit 的 `--read-only` 模式：

```rust
pub struct McpConfig {
    pub read_only: bool,  // 只暴露查询工具
    pub allowed_modules: HashSet<String>,  // 模块过滤
