...
versions:
  - version: 1
    ...
    author: ai
    context: "MCP 分析后发现 Sampling 未实现。这是 MCP 最强大的特性，允许 Server 反向请求 LLM 进行推理。用户确认需要 Sampling。"
    reason: "实现 Agentic 行为，Server 可请求 LLM 辅助决策"
...
**Sampling 是 MCP 最强大的特性** — 允许 Server 反向请求 LLM 进行推理，实现真正的 Agentic 行为。

### MCP 五大原语对比

| 原语 | 方向 | vnrs 状态 | 用途 |
|------|------|----------|------|
| Tools | LLM→Server | ✅ 13个 | 交易执行/查询 |
| Resources | Server→Client | ✅ 10个 | 实时数据暴露 |
| Prompts | 用户控制 | ❌ | 标准化分析模板 |
...
```
1. Tool 被调用时，需要 LLM 辅助决策
2. Tool 通过 ctx.session.create_message() 发送请求
3. Client 转发给 LLM（Claude/GPT等）
4. LLM 返回分析结果
5. Human-in-the-loop 可审批/拒绝
```
...
async fn handle_market_anomaly(&self, symbol: String, anomaly: String, ctx: Context) -> McpResult<String> {
    let prompt = format!("检测到 {} 异常: {}。当前持仓: {}。建议操作？", 
...
- [ ] 添加 Sampling 参数配置（max_tokens, temperature, model preference）
- [ ] 实现 human-in-the-loop 审批机制
- [ ] 添加 Sampling 调用日志和审计
- [ ] 示例 Tool: `analyze_sentiment` 使用 Sampling
- [ ] 示例 Tool: `suggest_strategy_params` 使用 Sampling
- [ ] 错误处理：LLM 不可用时的 fallback

## 安全考虑

1. **Human-in-the-loop**: 所有 Sampling 请求需用户审批
2. **速率限制**: 防止过度调用 LLM
3. **审计日志**: 记录所有 Sampling 请求和响应
4. **敏感信息**: 不在 prompt 中暴露 API key、密码等
