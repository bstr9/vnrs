...
versions:
  - version: 1
    ...
    reason: "模型生命周期管理是 AI 生产部署的核心需求"
    snapshot: "实现 ModelRegistry（SQLite 后端）和 ModelServer trait，支持 Development→Staging→Shadow→Canary→Production 阶段流转"
---

# ModelRegistry 模型注册与服务
...
- [ ] `OnnxModelServer` 实现（需 `ort` crate，可选 feature `ml-inference`）
- [ ] `ZmqModelServer` 实现（利用现有 RPC 基础设施）
- [ ] 可选 feature flags：`ml-inference`, `ml-local`, `ml-tract`, `ml-grpc`, `ml-full`

## 依赖

- `ort` crate（可选 feature `ml-inference`）
- `candle-*` crates（可选 feature `ml-local`）
- `tonic` + `prost`（可选 feature `ml-grpc`）
- **替代方案**：通过现有 ZMQ RPC 调用外部 Python 推理服务（零新依赖）
...
详见 `.sisyphus/plans/development-guide.md` 第五节 5.4 "AI-3 ModelRegistry 设计"
