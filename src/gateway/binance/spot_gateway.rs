...
impl BinanceSpotGateway {
                    ...
                    for f in filters {
                        ...
                        match filter_type {
                            ...
                            "NOTIONAL" | "MIN_NOTIONAL" => {
                                // Binance Spot uses "NOTIONAL", some exchanges use "MIN_NOTIONAL"
                                min_notional = f["minNotional"].as_str().unwrap_or("0").parse().unwrap_or(0.0);
                            }
                            _ => {}
                        }
                    }
    ...
    async fn start_user_stream(&self, proxy_host: &str, proxy_port: u16) -> Result<(), String> {
        ...
        let query = format!("apiKey={api_key}&timestamp={timestamp}");
...
impl BaseGateway for BinanceSpotGateway {
    ...
    async fn connect(&self, setting: GatewaySettings) -> Result<(), GatewayError> {
        ...
        *self.server.write().await = server.clone();
        ...
        let orders_trade = self.orders.clone();
        ...
        let on_disconnect_trade: Arc<dyn Fn() + Send + Sync> = Arc::new(move || {
            ...
            let subscription_id = subscription_id_reconnect.clone();
            let event_sender = event_sender_trade.clone();
            let orders = orders_trade.clone();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for orderid in stale_orderids {
                    ...
                    match rest_client_checker.get("/api/v3/order", &params, Security::Signed).await {
                        Ok(data) => {
