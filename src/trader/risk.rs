...
impl RiskManager {
    ...
    pub fn set_trading_state(&self, state: TradingState) {
        let mut current = self.trading_state.write().unwrap_or_else(std::sync::PoisonError::into_inner);
        let old = *current;
        *current = state;
        drop(current);
        if old != state {
    ...
    pub fn get_trading_state(&self) -> TradingState {
        *self.trading_state.read().unwrap_or_else(std::sync::PoisonError::into_inner)
    }
    ...
    pub fn check_order(&self, req: &OrderRequest) -> RiskCheckResult {
        ...
        {
            let state = *self.trading_state.read().unwrap_or_else(std::sync::PoisonError::into_inner);
            match state {
                TradingState::Halted => {
        ...
        if config.max_order_count > 0 {
            let active = self.active_orders.read().unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        if config.max_total_order_count > 0 {
            let total = *self.total_active_orders.read().unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        if config.max_daily_trades > 0 {
            let stats = self.daily_stats.read().unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        if req.order_type == super::constant::OrderType::Gtd {
            match req.expire_time {
                Some(expire_time) if expire_time <= Utc::now() => {
                    return RiskCheckResult::Rejected(format!(
                        "GTD order expire_time {expire_time:?} is in the past"
                    ));
                }
                None => {
    ...
    pub fn check_order_with_gateway(&self, req: &OrderRequest, gateway_name: &str) -> RiskCheckResult {
        ...
        if config.max_balance_usage_pct > 0.0 && req.price > 0.0 {
            let notional = req.volume * req.price;
            let balances = self.available_balance.read().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn record_trade(&self, trade: &TradeData) {
        ...
        let mut stats = self.daily_stats.write().unwrap_or_else(std::sync::PoisonError::into_inner);
        stats.trade_count += 1;
    ...
    pub fn update_account(&self, account: &AccountData) {
        ...
        {
            let mut prev = self.prev_balance.write().unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        {
            let mut balances = self.available_balance.write().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn order_submitted(&self, vt_symbol: &str) {
        {
            let mut active = self.active_orders.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            *active.entry(vt_symbol.to_string()).or_insert(0) += 1;
        }
        {
            let mut total = self.total_active_orders.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            *total += 1;
        }
    }
    ...
    pub fn get_daily_stats(&self) -> DailyStats {
        self.check_daily_reset();
        self.daily_stats.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone()
    }
}
