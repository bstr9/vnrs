...
pub fn check_optimization_setting(setting: &OptimizationSetting) -> Result<(), String> {
    ...
    if count > 100000 {
        tracing::warn!("警告：参数组合数量过大（{}组），可能导致内存不足", count);
    ...
    K: Fn(&HashMap<String, f64>) -> f64 + Send + Sync,
{
    ...
    let mut results: Vec<HashMap<String, f64>> =
        settings.into_par_iter().map(evaluate_func).collect();
    ...
    K: Fn(&HashMap<String, f64>) -> f64 + Send + Sync,
{
    ...
    for gen in 0..ngen {
        ...
        while new_population.len() < pop_size {
            let parent1 = &population[rng.random_range(0..mu)];
            let parent2 = &population[rng.random_range(0..mu)];

            let mut child_genes = parent1.genes.clone();

            // Crossover
            if rng.random::<f64>() < cxpb {
                let crossover_point = rng.random_range(0..child_genes.len());
                let end = child_genes.len();
                child_genes[crossover_point..]
                    .clone_from_slice(&parent2.genes[crossover_point..end]);
            }

            // Mutation
            if rng.random::<f64>() < mutpb {
                let mutation_point = rng.random_range(0..child_genes.len());
                let param_values = param_ranges[mutation_point];
                child_genes[mutation_point].1 =
                    param_values[rng.random_range(0..param_values.len())];
            }
