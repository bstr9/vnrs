...
impl BarManager {
    ...
    pub fn update_bar(&mut self, bar: BarData) {
        ...
        if !self.datetime_index_map.contains_key(&dt) {
            ...
            self.ordered_bars.sort_by_key(|b| b.datetime);
            self.datetime_index_map.clear();
            self.index_datetime_map.clear();
            for (ix, b) in self.ordered_bars.iter().enumerate() {
                self.datetime_index_map.insert(b.datetime, ix);
                self.index_datetime_map.insert(ix, b.datetime);
            }
