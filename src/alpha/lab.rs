...
impl AlphaLab {
    ...
    pub fn save_bar_data(&self, bars: Vec<AlphaBarData>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.logger
            .info(&format!("Saved {} bars to {}", bars.len(), file_path_clone));
        Ok(())
    }

    #[cfg(not(feature = "alpha"))]
    pub fn save_bar_data(
        &self,
        _bars: Vec<AlphaBarData>,
    ...
    pub fn load_contract_settings(
        &self,
    ) -> Result<HashMap<String, crate::trader::ContractData>, Box<dyn std::error::Error>> {
